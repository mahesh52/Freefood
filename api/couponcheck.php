...
$returnarr = array();
//echo "select * from freefood_coupons where coupon_code = '".$coupon_code."' ";
...
if($sql1 > 0){
	...
	if($sql100["valid_upto"] != "" && $sql100["assigned_to"] != "" && $sql100["products"] == "0"){
		...
		if(!empty($sql101["guid"])){	 
			 ...
			 $total_price = mysql_fetch_array(mysql_query("select sum(ctotal) from cart where session_id='".$user_id."' and order_id='' and pid>0")); 
	...
	if($sql2 == 0){
		...
		$sql4 = mysql_num_rows(mysql_query("select * from freefood_coupons_used where coupon_code= '".$coupon_code."' "));
		 ...
		 $chknew_new=mysql_num_rows(mysql_query("select * from cart where pid in ($products) and session_id='".$user_id."' and order_id='' and pid >0 "));
		 if($chknew_new > 0){
			...
			$chknew=mysql_query("select * from cart where pid in ($products) and session_id='".$user_id."' and order_id='' and pid >0 order by price desc limit 1 ");
