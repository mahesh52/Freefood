...
mysql_query("INSERT INTO `storeproducts` (`category` ,`subcategory` ,`name` ,`quantity`,`vprice` ,`description`,`status`,`date`,`sprice`,`mprice`,`state`,`area`,`zonal`,`town`) VALUES ('$category','$subcategory','$pname' ,'$quantity','$vprice' ,'$description','$status','$date','$sprice','$mprice','$state','$area','$zonal','$town')");
	}
	$pp=mysql_insert_id();
...
if(isset($_POST) && $_POST[hidid]!='')
{
	...
	$pname=str_replace("'","*_*",$pname);
	$description=str_replace("'","*_*",$description);
mysql_query("update `storeproducts` set `category`='$category' ,`subcategory`='$subcategory' ,`name`='$pname' ,`quantity`='$quantity',`vprice`='$vprice',`mprice`='$mprice' ,`sprice`='$sprice' ,`description`='$description',`status`='$status',`zonal`='$zonal',`town`='$town',`state`='$state',`area`='$area' where guid='$hidid'");
...
new nicEditor({fullPanel : true}).panelInstance('specifications');
    ...
    <body class="skin-black">
        <!-- header logo: style can be found in header.less -->
		 ...
		 <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
				...
				<form action="storeproducts.php" method="post" name="form1" id="frm1" onSubmit="return valid()" enctype="multipart/form-data">
      ...
      <input type="text" class="form-control" id="mrpprice" name="vprice" placeholder="Vendor Price" required onBlur="return sellprice(this.value);">
    </div>
                    </td></tr>
    ...
    <br clear="all" />
        ...
        <div id="vpb_upload_button">Browse Files</div>
               ...
               <tr><td colspan="2">                                                            
				...
				<form action="storeproducts.php" method="post" name="form1" id="frm1" onSubmit="return valid()">
               <table class="table table-responsive table-bordered">
<tr><td>State Partner</td>
                     <td><select class="form-control" name="state" required onChange="return subzone(this.value);">
             <option value="">Select State Partner</option>
...
<input type="checkbox" name="areas[]" value="<?php echo $state['guid'];?>" <?php if(in_array($state['guid'],$spt)){?> checked<?php } ?>><?php echo $state['name'];?></label>&nbsp;
    ...
    <br clear="all" />
        ...
        <div id="vpb_upload_button">Browse Files</div>
            ...
            <td><span id="subcat"><select class="form-control" name="subcategory">
             <option value="">Select Sub Category</option>
...
{
var sd1= (s*<?php echo $sellprice['company'];?>)/100;
var sd2= (s*<?php echo $sellprice['state'];?>)/100;
var sd3= (s*<?php echo $sellprice['zonal'];?>)/100;
var sd4= (s*<?php echo $sellprice['city'];?>)/100;
var sd5= (s*<?php echo $sellprice['clp'];?>)/100;
var sd6= (s*<?php echo $sellprice['misc'];?>)/100;
