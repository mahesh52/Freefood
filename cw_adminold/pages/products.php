...
mysql_query("INSERT INTO `products` (`category` ,`subcategory` ,`name` ,`quantity`,`vprice` ,`description`,`status`,`date`,`sprice`,`mprice`,`state`,`area`,`zonal`,`town`,`prebook`) VALUES ('$category','$subcategory','$pname' ,'$quantity','$vprice' ,'$description','$status','$date','$sprice','$mprice','$state','$area','$zonal','$town','$prebook')");
	}
	$pp=mysql_insert_id();
...
if(isset($_POST) && $_POST[hidid]!='')
{
	...
	$pname=str_replace("'","*_*",$pname);
	$description=str_replace("'","*_*",$description);
mysql_query("update `products` set `category`='$category' ,`subcategory`='$subcategory' ,`name`='$pname' ,`quantity`='$quantity',`vprice`='$vprice',`mprice`='$mprice' ,`sprice`='$sprice' ,`description`='$description',`status`='$status',`prebook`='$prebook',`zonal`='$zonal',`town`='$town',`state`='$state',`area`='$area' where guid='$hidid'");
