    ...
    <div data-page="store" class="page no-toolbar no-navbar">
        <div class="page-content">
            ...
            <div class="navbarpages whitebg bottomborder">
            	...
            	<h2 class="page_title2"><a href="home-coupons.php" class="back-btn-all"><i class="fa fa-angle-left" aria-hidden="true"></i>
                ...
                <div class="page_single layout_fullwidth_padding no-white-bg">
                    ...
                    <ul class="shop_items store-items-details">
                       ...
                       // $cart = mysql_num_rows(mysql_query("select * from cart where session_id='$_COOKIE[sessid]' and order_id='' and sid>0"));
                        //echo "select * from coupons where category='$_GET[gid]' and FIND_IN_SET('$_COOKIE[area]',area) ";exit;
                        $pro_total = mysql_num_rows(mysql_query("select * from coupons where category='$_GET[gid]' and FIND_IN_SET('$_COOKIE[area]',area) and quantity > 0 GROUP BY coupon_value,coupon_vendor  "));
                        if ($pro_total == 0) {
                            echo "<br><h1>No Coupons</h1>";
                        } else {
                            $qry = mysql_query("select * from coupons where category='$_GET[gid]' and FIND_IN_SET('$_COOKIE[area]',area) and quantity > 0 GROUP BY coupon_value,coupon_vendor  order by name asc") or die('error');
                            while ($row = mysql_fetch_assoc($qry)) {
                                $chk = mysql_fetch_array(mysql_query("select * from cart where cid='$row[guid]' and session_id='$_COOKIE[sessid]' and order_id=''"));
                                $img = mysql_fetch_array(mysql_query("select * from imagefiles where coupon_id='$row[coupon_vendor]'"));
                                $fqty = 0;
                                $chkqty[0] = 0;
                                $cartqty[0] = 0;
                               //echo  $chkqty[0] = $row["quantity"];
							  // echo "select sum(cquantity) from cart where cid='$row[guid]' and order_id!=''";
                              //   $cartqty = mysql_fetch_array(mysql_query("select sum(cquantity) from cart where cid='$row[guid]' and order_id!=''"));
							//	echo  $cartqty[0];
                                 $fqty = $row["quantity"];
                                $couponvendor = mysql_fetch_array(mysql_query("select * from couponsvendor where guid='$row[coupon_vendor]' "));
                                ?>
                                <li>
                                    <div class = "shop_thumb"><a  href="couponitem.php?gid=<?php echo $row['guid']; ?>">
                                            <img height ="120px" width = "120px" src="uploaded_files/<?php echo $img['image']; ?>" alt = "" title = "" />
                                        </a>
                                    </div>
                                    <div class = "shop_item_details">
                                        <h4><a href="couponitem.php?gid=<?php echo $row['guid']; ?>">
                                            <?php echo $couponvendor['name']; ?> <br> Coupon Worth of   <?php echo $row['coupon_value']; ?>/-</a>
                                        </h4>
                                        <div class = "shop_item_price"><i class="fa fa-inr"></i><?php echo $row['sprice']; ?></div>
                                        <?php if($row['validity'] != ""){ ?> <div class = "shop_item_price">Valid For <?php echo $row['validity']; ?> Months</div> <?php } ?>
...
?>
                    ...
                    <div id="footer" class="shop_pagination" <?php if ($total == 0) { ?>style="display:none;"<?php } ?>>
                        <a href="coupon-checkout.php"  class="prev_shop"> Proceed <i aria-hidden="true" class="fa fa-angle-right"></i></a>
