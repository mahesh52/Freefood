...
{
	$chk=mysql_fetch_array(mysql_query("select * from cart where cid='$guid' and session_id='$_COOKIE[sessid]' and order_id=''"));
	if(empty($chk['guid']))
	{
mysql_query("INSERT INTO `cart` (`cid` ,`cquantity` ,`price` ,`ctotal` ,`session_id` ,`date`) VALUES ('$guid', '$qty', '$price', '$ctotal', '$_COOKIE[sessid]', '$date')");
	}
	else
	{
	...
	$cart_total=mysql_fetch_array(mysql_query("select sum(ctotal) from cart where session_id='$_COOKIE[sessid]' and order_id='' and cid>0"));
