...
$prod_detail=mysql_fetch_array(mysql_query("select * from coupons where guid='$_GET[gid]'"));
?>
  <div class="pages">
  <div data-page="storeitem" class="page no-toolbar no-navbar">
	...
	<div class="navbarpages whitebg bottomborder">
        ...
        <i class="fa fa-angle-left" aria-hidden="true"></i></a>
                            <span class="page-title-head">&nbsp;&nbsp;Coupons</span></h2>
	...
	<div class="page_single layout_fullwidth_padding product-description-title">
      ...
      <div class="shop_item">
<?php $prod_detail=mysql_fetch_array(mysql_query("select * from coupons where guid='$_GET[gid]'"));
$img = mysql_fetch_array(mysql_query("select * from imagefiles where coupon_id='$prod_detail[coupon_vendor]'"));
	//$img=mysql_fetch_array(mysql_query("select * from imagefiles where sid='$prod_detail[guid]'"));
	$chk=mysql_fetch_array(mysql_query("select * from cart where cid='$prod_detail[guid]' and session_id='$_COOKIE[sessid]' and order_id=''"));
	 $total=0; 
					...
					$fqty = 0;
                                $chkqty[0] = 0;
                                $cartqty[0] = 0;
                              //  $chkqty[0] = $prod_detail["quantity"];
                              //  $cartqty = mysql_fetch_array(mysql_query("select sum(cquantity) from cart where cid='$_GET[gid]' and order_id!=''"));
                                $fqty = $prod_detail["quantity"];
					$cart=mysql_num_rows(mysql_query("select * from cart where session_id='$_COOKIE[sessid]' and order_id='' and cid>0"));?>
		...
		<input type="hidden" name="cart" id="cart" class="cart" value="<?php echo $cart;?>">
          ...
          <a <?php if ($total == 0) { ?>style="display:none;"<?php } ?> id="footer" href="coupon-checkout.php" class="button_full btyellow"  >Check Out</a>
