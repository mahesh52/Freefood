...
{
mysql_query("delete from storevendor where guid='$_GET[deleteid]'");
header('location:storevendors.php');	
}
if(isset($_POST) && $_POST['state']!='' && $_POST['Submit']=='Addvendor')
{$area='';
	$chk=mysql_fetch_array(mysql_query("select * from storevendor where email='$email'"));
	if($state!='' && $chk['guid']=='')
	{
...
mysql_query("INSERT INTO `storevendor` (`name` ,`state`,`area`,`mobile`,`email`,`password`,`address`,`pstatus`,`status`,`zonal`,`town`) VALUES ('$name','$state','$area','$mobile','$email','$password','$address','Pending','$status','$zonal','$town')");
...
mysql_query("update `storevendor` set `name`='$name',`state`='$state',`area`='$area',`mobile`='$mobile',`email`='$email',`password`='$password',`address`='$address',`status`='$status',`zonal`='$zonal',`town`='$town' where guid='$hidid'");
header('location:storevendors.php');	
}
...
if(isset($_POST) && $_POST['submit']=='Update')
{
	...
	{
	$chk=mysql_fetch_array(mysql_query("select * from store_vendor_products where vendor_id='$gid' and product_id='$product_id[$i]'"));
	if($chk['guid']=='')
	{
	if($price[$i]>0)
	{
      mysql_query("INSERT INTO  `store_vendor_products` (`vendor_id` ,`product_id` ,`price` ,`date`) VALUES ('$gid',  '$product_id[$i]',  '$price[$i]',  '$date')");
	}
	}
	else
	{
	...
	mysql_query("update `storevendor` set `pstatus`='$pstatus' where guid='$gid'");
	?><script>
alert("Updated Successfully");
window.location='storevendors.php';
</script><?php 	
}
?><!DOCTYPE html>
<html>
        ...
        <!-- header logo: style can be found in header.less -->
		 ...
		 <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
				...
				<form action="storevendors.php" method="post" name="form1" onSubmit="return valid()" enctype="multipart/form-data">
             ...
             <option value="">Select Area</option>
               ...
               <tr><td colspan="2">                                                            
				...
				<?php }elseif(isset($_GET) && $_GET['action']=='products'){
					$vendor=mysql_fetch_array(mysql_query("select * from storevendor where guid='$_GET[gid]'"));?>
                <form method="post" action="storevendors.php">
                 <table class="table table-responsive table-bordered">
...
$query_clients="SELECT * FROM `storeproducts` order by guid desc  ";
...
$m=1;
						    while($client_count = mysql_fetch_assoc($query_clients)){
								$chk=mysql_fetch_array(mysql_query("select * from store_vendor_products where vendor_id='$_GET[gid]' and product_id='$client_count[guid]'"));
...
<tr><td colspan="10"><button type="submit" name="submit" value="Update" class="btn btn-success">Update</button></td></tr>
					...
					$det=mysql_fetch_array(mysql_query("select * from storevendor where guid='$guid'"));?>
				
				<form action="storevendors.php" method="post" name="form1" onSubmit="return valid()">
               <table class="table table-responsive table-bordered">
...
<input type="checkbox" name="areas[]" value="<?php echo $state['guid'];?>" <?php if(in_array($state['guid'],$spt)){?> checked<?php } ?>><?php echo $state['name'];?></label>&nbsp;
               ...
               <input type="hidden" name="hidid" value="<?php echo $det['guid'];?>">                                                          
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="storevendors.php?action=add"><button type="button" class="btn btn-success">Add Vendor</button></a>
...
$query_clients="SELECT * FROM `storevendor`  order by guid desc  Limit $start, $limit ";

$query_clients=mysql_query($query_clients);

$select1="SELECT * FROM `storevendor`";
...
@$divs=$total%$limit;
	...
	@$totals=(int)($total/$limit) + 1;
						    ...
						    while($client_count = mysql_fetch_assoc($query_clients)){
								...
								//echo "select * from store_vendor_products where vendor_id='$client_count[guid]'";
								$chk=mysql_num_rows(mysql_query("select * from store_vendor_products where vendor_id='$client_count[guid]' and product_id in (select guid from storeproducts )"));?>
                        	...
                        	<tr>
                               ...
                               <td><a href="storevendors.php?gid=<?php echo $client_count['guid'];?>&action=products">Total : <?php echo $chk;?></a></td>
                               <td><a href="storevendors.php?gid=<?php echo $client_count['guid'];?>&action=products"><?php echo $client_count['pstatus'];?></a></td>
                               <td><a href="storevendors.php?action=edit&guid=<?php echo $client_count['guid']; ?>"><button type="button" class="btn btn-default">Edit</button></a></td>
                                <td><a href="storevendors.php?action=delete&deleteid=<?php echo $client_count['guid']; ?>" onClick="return delete1();"><button type="button" class="btn btn-danger">Delete</button></a></td>
