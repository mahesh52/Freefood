...
mysql_query("INSERT INTO `vendor` (`name` ,`state`,`area`,`mobile`,`email`,`password`,`address`,`pstatus`,`status`,`zonal`,`town`,`prebook`,vendor_type) VALUES ('$name','$state','$area','$mobile','$email','$password','$address','Pending','$status','$zonal','$town','$prebook','$vendor_type')");
...
mysql_query("update `vendor` set `name`='$name',`state`='$state',`area`='$area',`mobile`='$mobile',`email`='$email',`password`='$password',`address`='$address',`status`='$status',`zonal`='$zonal',`town`='$town',`prebook`='$prebook',vendor_type = '$vendor_type' where guid='$hidid'");
header('location:vendors.php');	
}
            ...
            <tr><td>Area</td>
             ...
             <option value="">Select Area</option>
					...
					<tr><td>Vendor Type</td><td>
                        <input type="radio" name="vendor_type"  value="Food" checked/> Food
                        <input type="radio" name="vendor_type"  value="ERFV" /> ERFV
                    </td></tr>
               <tr><td colspan="2">                                                            
...
$m=1;
						    while($client_count = mysql_fetch_assoc($query_clients)){
								if($vendor['vendor_type'] == "Food"){
									...
									//echo "select available_qty as price from erfvstock where product_id='$client_count[guid]' and erfv_vendor='$_GET[gid]'";
									$chk = mysql_fetch_array(mysql_query("select available_qty as price from erfvstock where product_id='$client_count[guid]' and erfv_vendor='$_GET[gid]'"));
								}
                        	...
                        	<tr>
                            	...
                            	<td><?php echo $m; ?></td>
                                 ...
                                 <td><?php echo $cat['name'];?></td>
								  ...
								  <td>
								  <?php if($vendor['vendor_type'] == "Food"){ ?>
...
<input type="checkbox" name="areas[]" value="<?php echo $state['guid'];?>" <?php if(in_array($state['guid'],$spt)){?> checked<?php } ?>><?php echo $state['name'];?></label>&nbsp;
                <?php } ?></span></td></tr>
					...
					<tr><td>Vendor Type</td><td>
                        <input type="radio" name="vendor_type"  value="Food" <?php if($det['vendor_type']=='Food'){echo "checked";}?>/> Food
                        <input type="radio" name="vendor_type"  value="ERFV" <?php if($det['vendor_type']=='ERFV'){echo "checked";}?> /> ERFV
                    </td></tr>
               <tr><td colspan="2">  
...
@$divs=$total%$limit;
	...
	@$totals=(int)($total/$limit) + 1;
						    ...
						    while($client_count = mysql_fetch_assoc($query_clients)){
								...
								if($client_count['vendor_type'] == "Food"){ 
								$chk=mysql_num_rows(mysql_query("select * from vendor_products where vendor_id='$client_count[guid]'"));
								}else{
								$chk = mysql_num_rows(mysql_query("select * from erfvstock where  erfv_vendor='".$client_count['guid']."'"));
								}
								?>                   
								<tr>
							   ...
							   <td><?php echo $client_count['vendor_type'];?></td>
                               <td><?php echo $client_count['status'];?></td>
                               <td>
							   <?php if($client_count['vendor_type'] == "Food"){ ?>
							   <a href="vendors.php?gid=<?php echo $client_count['guid'];?>&action=products">Total : 
							   <?php echo $chk;?></a>
							   <?php }else{ ?>
							   <a href="vendors.php?gid=<?php echo $client_count['guid'];?>&action=products">Total : 
								   <?php echo $chk;?>
                               ...
                               <td>
							   <?php if($client_count['vendor_type'] == "Food"){ ?>
							   <a href="vendors.php?gid=<?php echo $client_count['guid'];?>&action=products">
							   <?php echo $client_count['pstatus'];?>
							   </a>
							   <?php }else{ ?>
								   <?php echo "N/A";?>
							   <?php } ?>
