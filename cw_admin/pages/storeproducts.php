...
if (isset($_POST) && $_POST[category] != '' && $_POST[hidid] == '') {
    ...
    if ($category != '') {
        ...
        mysql_query("INSERT INTO `storeproducts` (`category` ,`name` ,`quantity`,`vprice` ,`description`,`status`,`date`,`sprice`,`mprice`,`state`,`area`,`zonal`,`town`,vat) VALUES ('$category','$pname' ,'$quantity','$vprice' ,'$description','$status','$date','$sprice','$mprice','$state','$area','$zonal','$town','$vat')");
    }
    $pp = mysql_insert_id();
...
if (isset($_POST) && $_POST[hidid] != '') {
    ...
    $pname = str_replace("'", "*_*", $pname);
    $description = str_replace("'", "*_*", $description);
    mysql_query("update `storeproducts` set `category`='$category' ,`name`='$pname' ,`quantity`='$quantity',`vprice`='$vprice',`mprice`='$mprice' ,`sprice`='$sprice' ,`description`='$description',`status`='$status',`zonal`='$zonal',`town`='$town',`state`='$state',`area`='$area',vat='$vat' where guid='$hidid'");
...
<html>
    ...
    <!--                            <tr>
                            ...
                            <tr><td><label>Min Selling Price</label></td>
                                ...
                                </td></tr><tr><td><label>VAT</label></td>
                       <td><div class="input-group">
    ...
    <!--                            <tr><td><label>Sub Category</label></td><td>
      ...
      <input type="text" class="form-control" id="vat" name="vat" placeholder="VAT in %" value="<?php echo $det['vat'];?>"  required>
                            ...
                            <tr><td colspan="2"><?php
                                    $qry = mysql_query("select * from imagefiles where sid='$det[guid]'");
                                    while ($row = mysql_fetch_assoc($qry)) {
                        ...
                        <form method="get" action="storeproducts.php"><table class="table table-responsive table-bordered">
                                <tr><td><select class="form-control" name="category" required onChange="return subcat(this.value);">
                                    ...
                                    <td><span id="subcat"><select class="form-control" name="subcategory">
                                                <option value="">Select Sub Category</option>
                                                <?php
                                                $qry = mysql_query("select * from subcategory where refid='$category' order by name asc");
                                                while ($row = mysql_fetch_assoc($qry)) {
                                            ...
                                            function sellprice()
                                            {
                                                ...
                                                if (mrpprice != "" && price != "") {
                                                    if (parseFloat(price) > parseFloat(mrpprice)) {
                                                        ...
                                                        $.ajax({
                                                            type: "post",
                                                            url: "getMinSellingPrice.php",
                                                            data: "val=" + mrpprice + "&price=" + price,
                                                            success: function (response) {
