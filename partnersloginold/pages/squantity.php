...
if(isset($_POST) && $_POST['submit']=='Update')
{
	...
	{
	 mysql_query("update `store_vendor_quantity` set `quantity`='$quantity[$i]' where guid='$chkqty[guid]'");	
	}
	}
...
@$divs=$total%$limit;
	...
	@$totals=(int)($total/$limit) + 1;
						    ...
						    while($client_count = mysql_fetch_assoc($query_clients)){
								...
								if($chk['price']>0){$price=$chk['price'];if($chkqty[0]>0){$quantity=$chkqty[0]-$cartqty[0];}else{$quantity=0;}
