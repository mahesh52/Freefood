...
if($balance>$amount)
{
mysql_query("INSERT INTO  `comwallet` (`userid` ,`points` ,`pstatus` ,`date`,`status`,`description`) VALUES ('$_SESSION[partner_loginid]', '$amount',  'Pending',  '$date1',  'Debit','$_SESSION[partner_status]')");	
}
                  ...
                  <?php if($_GET['action']=='add'){
					  ...
					  if($_SESSION['partner_status']=='State Partner'){
						  ...
						  $det=mysql_fetch_array(mysql_query("select * from cluster where guid='$_SESSION[partner_loginid]'"));
                                                   ...
                                                   if($_SESSION['partner_status']=='Coupons Vendor'){
						  $det=mysql_fetch_array(mysql_query("select * from couponsvendor where guid='$_SESSION[partner_loginid]'"));
...
//echo "select * from cat  Limit $start, $limit";
 ...
 $query_clients="SELECT * FROM `comwallet` where userid='$_SESSION[partner_loginid]' and status='Debit' and description='$_SESSION[partner_status]' order by guid desc  Limit $start, $limit ";

$query_clients=mysql_query($query_clients);

$select1="SELECT * FROM `comwallet` where userid='$_SESSION[partner_loginid]' and status='Debit' and description='$_SESSION[partner_status]' ";
