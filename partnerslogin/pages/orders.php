        ...
        <?php if ($prods['price'] == '') {
            ...
            $price = $prods['price'];
                                                    ...
                                                    <td class="align_center vline"><?php echo $prods['cquantity']; ?>
                                                        <input type="hidden" name="hidid[]" value="<?php echo $prods[guid]; ?>"></td>
                                                    <td class="align_center vline"><?php echo $price; ?></td>
													<td class="align_center vline"><?php echo $prods['vat']; ?></td>
                                                    <td class="align_center vline"><?php echo $prods['cquantity'] * $price; ?></td>
													<?php 
                        ...
                        <table class="table table-responsive table-bordered">
                            ...
                            <thead>
                                ...
                                <?php if ($_GET['st'] != 'Food') { ?> <th>Products</th><?php } ?>
                                    <th>Details</th>
                                   <?php if($_SESSION['partner_status']!="Food Vendor"){?> <th>Total Amount</th><?php } ?>
								   <?php if($_SESSION['partner_status']!="Food Vendor"){?> <th>Discount</th><?php } ?>
                                    <th>Order Status</th>
                                    <?php if($_SESSION['partner_status']!="Food Vendor"){?><th>Print</th><?php } ?>
...
//echo $query_clients="SELECT * FROM `orders` a,cart b  where a.guid = b.order_id and gettime='$_GET[st]' and b.clp_id='$details[guid]' order by delivery_time asc  Limit $start, $limit ";
                                if($_GET["st"] == "Food"){
									 if($_SESSION['partner_status']=="Food Vendor"){
                                       $query_clients = "SELECT a.guid as order_id,a.*,b.*,b.guid as guid FROM `orders` a,cart b where a.guid = b.order_id and gettime='$_GET[st]' and b.vendor_id='$details[guid]' and order_status in ('Accepted','Packed','Delivering','Delivered')
     order by b.guid desc Limit $start, $limit";
                                $select1 = "SELECT * FROM `orders` a,cart b where a.guid = b.order_id and gettime='$_GET[st]' and b.vendor_id='$details[guid]'";
									 }
									 else{
										 $query_clients = "SELECT a.guid as order_id,a.*,b.*,b.guid as guid FROM `orders` a,cart b where a.guid = b.order_id and gettime='$_GET[st]' and b.clpid='$details[guid]' and order_status in ('Accepted','Packed','Delivering','Delivered')
     order by b.guid desc Limit $start, $limit";
                                $select1 = "SELECT * FROM `orders` a,cart b where a.guid = b.order_id and gettime='$_GET[st]' and b.clpid='$details[guid]'";
									 }
                                }
                                else{
                                       $query_clients = "SELECT * FROM `orders` as a where gettime='$_GET[st]' and vendor_id='$details[guid]'   order by guid desc  Limit $start, $limit ";
        ...
        date('G:i A', strtotime("+150 minutes", strtotime($client_count['delivery_time'])));
            ...
            echo "btn-danger";
        } ?>"><?php if ($client_count['order_status'] == "Accepted") {
            echo "Not Delivered";
        } else {
		...
		</td><?php if($_SESSION['partner_status']!="Food Vendor"){?>
                                        <td>
        ...
        echo $check['state']; ?></td>
                                            <?php if ($_GET['st'] == 'Food') { ?><td colspan="2" valign="top"><table width="100%" border="1"><tr><td valign="top" width="50%"><strong>Products</strong></td>
                                                ...
                                                <?php echo $n;
                                                echo ". ";
                                                echo $products['name']; ?>(<?php echo $row2['cquantity']; ?>)<br><?php } ?></td>
        ...
        <?php } ?></td>
            ...
            echo $client_count['getdate'];
                                        ...
                                        <td><a href="orders.php?action=edit&st=<?php echo $_GET['st']; ?>&gid=<?php echo $client_count['guid']; ?>" class="btn <?php if ($client_count['status'] == 'Delivered') {
            echo "btn-success";
        } else {
            echo "btn-danger";
        } ?>"><?php echo $client_count['status']; ?></a></td>
