...
$query_clients="SELECT * FROM `comwallet` where (date(date) Between '$from' and '$to') and userid='$_SESSION[partner_loginid]' and description='$_SESSION[partner_status]' order by guid desc  Limit $start, $limit ";
$select1="SELECT * FROM `comwallet` where (date(date) Between '$from' and '$to') and userid='$_SESSION[partner_loginid]' and description='$_SESSION[partner_status]'";
$cmn="SELECT sum(points) FROM `comwallet` where (date(date) Between '$from' and '$to') and userid='$_SESSION[partner_loginid]' and description='$_SESSION[partner_status]'";	
}
else
{
...
$cmn="SELECT sum(points) FROM `comwallet` where userid='$_SESSION[partner_loginid]' and description='$_SESSION[partner_status]' and status = 'Credit'";
$cmn1="SELECT ifnull(sum(points),0) FROM `comwallet` where userid='$_SESSION[partner_loginid]' and description='$_SESSION[partner_status]' and status = 'Debit' ";
}
...
$commission_deb=mysql_fetch_array(mysql_query($cmn1));
...
@$divs=$total%$limit;
	...
	}?><p><strong>Total Commission Amount : <?php if($commission[0]==''){echo "0";}else{echo $commission[0];}?>/-</strong>
          
        </p><p>
          <strong>Total Withdrawal Amount : <?php if($commission_deb[0]==''){echo "0";}else{echo $commission_deb[0];}?>/-</strong>
        </p>
                   <table class="table table-responsive table-bordered">
