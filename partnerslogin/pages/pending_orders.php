...
{  $wat=0;
	...
	{   
		...
		$are_dets = mysql_fetch_array(mysql_query("select * from register where guid='$order[userid]'"));
		$area = $are_dets["state"];
		
		$food_vendor = mysql_fetch_array(mysql_query("select * from vendor where FIND_IN_SET('".$area."',`area`) "));
	 
		//echo "update cart set order_status='Accepted',clpid='$reg[clp]',vendor_id='$_SESSION[vendor_loginid]' where guid='$gid'";exit;
		mysql_query("update cart set order_status='Accepted',clpid='$reg[clp]',vendor_id='$_SESSION[vendor_loginid]' where guid='$gid'");
		mysql_query("update orders set vendor_id='$vid',clp_id='$reg[clp]' where guid='$order[guid]'");
		$chk=mysql_fetch_array(mysql_query("select * from clpwallet where userid='$reg[clp]' and refid='$gid'"));
		if(empty($chk['guid']) && $reg['clp']!='')
		{
	mysql_query("INSERT INTO  `clpwallet` (`userid` ,`points` ,`refid` ,`date`,`status`) VALUES ('$reg[clp]', '$det[ctotal]',  '$gid',  '$date1',  'Debit')");
	$ins=mysql_insert_id();
	if($ins>0)
	{
	...
	$zonal=mysql_fetch_array(mysql_query("select * from zonal where guid='$town[zonal]'"));
		...
		$erfv_cmn=($com['erfv']*$det['ctotal'])/100;
...
//if($food_vendor["vendor_type"] == "ERFV"){
	if($erfv_cmn>0){

mysql_query("INSERT INTO  `comwallet` (`userid` ,`points` ,`refid` ,`date`,`status`,`description`) VALUES ('$food_vendor[guid]', '$clp_cmn',  '$ins',  '$date1',  'Credit','Cluster Partner')");
...
$query_clients="SELECT * FROM `cart` where order_status='Confirmed' and area in ($area) and date(delivery_time) >= date(now())  order by delivery_time asc  Limit $start, $limit ";

$query_clients=mysql_query($query_clients);

$select1="SELECT * FROM `cart` where order_status='Confirmed' and area in ($area)";
...
@$divs=$total%$limit;
	...
	@$totals=(int)($total/$limit) + 1;
							...
							$sql_erfv = mysql_fetch_array(mysql_query("select * from vendor where guid = '$_SESSION[vendor_loginid]'"));
							...
							if($vendor_type == "ERFV"){
												$vqty = mysql_fetch_array(mysql_query("select available_qty from erfvstock where product_id='$client_count[pid]' and erfv_vendor='$_SESSION[vendor_loginid]'"));
											}
											else{
		 ...
		 if($vendor_type == "ERFV"){
												$chkqty = mysql_fetch_array(mysql_query("select available_qty from erfvstock where product_id='$client_count[pid]' and erfv_vendor='$_SESSION[vendor_loginid]'"));
											}
											else{
		 ...
		 $cartqty=mysql_fetch_array(mysql_query("select sum(cquantity) from cart where pid='$client_count[pid]' and date='$date' and order_id!='' and vendor_id='$_SESSION[vendor_loginid]' "));
...
<td>ORD<?php echo $client_count['order_id']; //echo "---";echo $fqty;?></td>
           ...
           <?php echo $products['name'];?>(<?php echo $client_count['cquantity']; ?>)<br>
            ...
            <td><?php echo date('d-M-Y G:i A', strtotime($client_count['delivery_time']));   ?>- <b><?php echo 
            date('G:i A', strtotime("+150 minutes", strtotime($client_count['delivery_time']))); ?></b></td>
            <td>Spicy : <b><?php echo $client_count['spicy_level'];   ?></b></td>
