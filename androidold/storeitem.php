...
<body>
    ...
    <section class="content-part animsition item bg-cyan" data-animsition-in-class="fade-in-up-sm" data-animsition-out-class="fade-out-up-sm" data-animsition-in-duration="1000">
        <article>
         ...
         $chkqty=mysql_fetch_array(mysql_query("select sum(quantity) from store_vendor_quantity where product_id='$prod_detail[guid]'"));
		 $cartqty=mysql_fetch_array(mysql_query("select sum(cquantity) from cart where sid='$prod_detail[guid]' and order_id!=''"));
