		 ...
		 $total_checkout=mysql_fetch_array(mysql_query("select sum(ctotal) from cart where session_id='$_COOKIE[sessid]' and order_id='' and sid>0"));?>  
                    <ul class="list-group">
                      ...
                      <li class="list-group-item list-group-item-success">
                          ...
                          <span class="pull-right"><i class="fa fa-int"></i> <?php echo $total_checkout[0];?></span>
                          <div class="clearfix"></div>
                      </li>
                       <li class="list-group-item list-group-item">
                          <span class="pull-left">Discount</span>
                          <span class="pull-right"><i class="fa fa-int"></i> <?php $dis=0;
						  if($points!=''){
							  if($points=='yes'){ if($total_checkout[0]>$wat){ $dis=$wat;}else{ $dis=$total_checkout[0];}}
							  if($points=='special'){ if($total_checkout[0]>$wat1){$dis=$wat1;}else{$dis=$total_checkout[0];}}
							  }else{ $dis=0;}echo $dis;$pay=0;$brn=0;
							  if($dis>0){$pay=$total_checkout[0]-$dis;}else{$pay=$total_checkout[0];}
						   ?></span>
                          <div class="clearfix"></div>
                      </li>
                      <?php /*$burn=mysql_fetch_array(mysql_query("select * from commissions order by guid desc limit 0,1"));
					  if($burn['burnbudget']>0)
					  {?>
                      <li class="list-group-item list-group-item">
                          <span class="pull-left">Special Discount</span>
                          <span class="pull-right"><i class="fa fa-int"></i> <?php 
						  $brn=($pay*$burn['burnbudget'])/100;
						  echo $brn;
						   ?></span>
                          <div class="clearfix"></div>
                      </li><?php }*/ ?>
                      <li class="list-group-item list-group-item-success">
                          ...
                          <span class="pull-right"><i class="fa fa-int"></i> <?php  echo $pay;?></span>
                          <div class="clearfix"></div>
                      </li>
                    </ul>
                </div>
