...
$login=mysql_fetch_array(mysql_query("select * from `cluster` where `guid`='$_SESSION[clp_loginid]'"));
	...
	if($login['password']==$oldpassword)
{
		...
		//$decrypted = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5($key), base64_decode($encrypted), MCRYPT_MODE_CBC, md5(md5($key))), "\0");
			...
			mysql_query("UPDATE `cluster` set `password`='$password' where `guid`='$_SESSION[clp_loginid]'");
