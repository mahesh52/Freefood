...
if(isset($_POST) && $_POST['city']!='' && $_POST['hidid']=='')
{
	$chk=mysql_fetch_array(mysql_query("select * from vendor where email='$email'"));
	if($city!='' && $area!='' && $chk['guid']=='')
	{
mysql_query("INSERT INTO `vendor` (`name` ,`city`,`area`,`mobile`,`email`,`password`,`address`) VALUES ('$name','$city','$area','$mobile','$email','$password','$address')");
	}
...
{
mysql_query("update `vendor` set `name`='$name',`city`='$city',`area`='$area',`mobile`='$mobile',`email`='$email',`password`='$password',`address`='$address' where guid='$hidid'");
header('location:vendors.php');	
}
?><!DOCTYPE html>
<html>
		 ...
		 <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
               ...
               <table class="table table-responsive table-bordered">
                     ...
                     <tr><td>City</td>
                     <td><select class="form-control" name="city" required onChange="return subcategories(this.value);">
             <option value="">Select City</option>
              <?php $qry=mysql_query("select * from city order by name asc");
            ...
            <tr><td>Area</td>
             ...
             <option value="">Select Area</option>
               ...
               <tr><td colspan="2">                                                            
                    <button type="submit" name="Submit" value="Update" class="btn bg-yellow btn-block">Add Vendor</button>  
                </td></tr></table>
            </form>
            ...
            <tr><td>Area</td>
                     <td><span id="subcagtegory"><select class="form-control" name="area" required>
             <option value="">Select Area</option>
              <?php $qry=mysql_query("select * from area where refid='$det[city]' order by name asc");
			  while($row=mysql_fetch_assoc($qry))
			  {
				?><option value="<?php echo $row['guid'];?>" <?php if($det['area']==$row['guid']){?> selected<?php } ?>><?php echo $row['name'];?></option><?php
			  }?>            	 	
			</select></span></td></tr>
           
                    <tr><td>Vendor Name</td><td>
                        <input type="text" name="name" class="form-control" placeholder="Vendor Name" value="<?php echo $det['name'];?>"/>
                    </td></tr>
                    <tr><td>Mobile</td><td>
               ...
               <input type="hidden" name="hidid" value="<?php echo $det['guid'];?>">                                                          
                    <button type="submit" name="Submit" value="Update" class="btn bg-yellow btn-block">Update Vendor</button>  
                </td></tr></table>
            </form><?php }else{?>
...
@$divs=$total%$limit;
	...
	@$totals=(int)($total/$limit) + 1;
						    ...
						    while($client_count = mysql_fetch_assoc($query_clients)){
								...
								$det1=mysql_fetch_array(mysql_query("select * from area where guid='$client_count[area]'"));?>
