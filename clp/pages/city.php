...
{
mysql_query("INSERT INTO `city` (`name`) VALUES ('$event')");
header('location:city.php');	
}
if(isset($_POST) && $_POST[hidid]!='')
{
mysql_query("update `city` set `name`='$event' where guid='$hidid'");
header('location:city.php');	
}
?><!DOCTYPE html>
<html>
...
@$divs=$total%$limit;
	...
	@$totals=(int)($total/$limit) + 1;
						    ...
						    while($client_count = mysql_fetch_assoc($query_clients)){
								//$check=mysql_fetch_array(mysql_query("select * from city where guid='$client_count[referral]'"));
								?>
