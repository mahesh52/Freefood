...
$query_clients="SELECT distinct(order_id) FROM `cart` where clpid='$_SESSION[clp_loginid]'  order by guid desc  Limit $start, $limit ";

$query_clients=mysql_query($query_clients);

$select1="SELECT distinct(order_id) FROM `cart` where clpid='$_SESSION[clp_loginid]'";
...
@$divs=$total%$limit;
	...
	@$totals=(int)($total/$limit) + 1;
						    ...
						    while($client_count = mysql_fetch_assoc($query_clients)){$timing='';
								$order=mysql_fetch_array(mysql_query("select * from orders where guid='$client_count[order_id]'"));
...
<td>ORD<?php echo $client_count['order_id'];?></td>
                               ...
                               <td colspan="2" valign="top"><table width="100%" border="1"><tr><td valign="top" width="50%"><strong>Products</strong></td>
			  ...
			  $cart=mysql_query("select * from cart where vendor_id='$prods[vendor_id]' and order_id='$client_count[order_id]'");
...
<?php }?></td><?php } ?></tr></table></td>
            ...
            <td><?php echo $order['pmode'];?>
            </td>
            <td><?php echo $order['total']; ?></td>
            <td><a href="orders.php?action=edit&st=<?php echo $_GET['st'];?>&gid=<?php echo $order['guid']; ?>" class="btn <?php if($order['status']=='Delivered'){echo "btn-success";}else{echo "btn-danger";}?>"><?php echo $order['status'];?></a></td>
            <td><a href="../../print.php?gid=<?php echo $order['guid']; ?>" target="_blank" class="btn btn-warning">Print</a></td>
                                </tr>
